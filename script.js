const datiGallagher = {
    ...
    'aspetto': ["Aspetto Fisico", `Corporatura: È un uomo dall'ossatura robusta e dalla presenza imponente, ma non massiccia in senso atletico. La sua figura trasmette un senso di pesantezza e solidità, come quella di un veterano che ha passato anni sotto le armi o a svolgere lavori logoranti. Ha una postura leggermente incurvata, tipica di chi porta un peso invisibile sulle spalle. Altezza: È decisamente alto, sopra la media. Occhi: Lo sguardo è il suo tratto più distintivo, carichi di malinconia e stanchezza.`, "https://res.cloudinary.com/dqoncufhc/image/upload/v1768446404/ricordo_znoads.png"],
    'carattere': ["Il Carattere", `L’Uomo del "Vero e Falso": Gallagher è intrinsecamente un bugiardo, ma non per malvagità. Lui stesso si definisce un "impasto di bugie". La sua natura è quella di un uomo che maschera la verità per proteggere segreti più grandi. Stanchezza Esistenziale: Il tratto più profondo è il suo essere "stanco del mondo".`, "https://res.cloudinary.com/dqoncufhc/image/upload/v1768446404/ricordo2_ysvgil.png"]
};
...
function apri(chiave) {
    ...
    var extra = document.getElementById('img-extra-modale');
    ...
    } else if(d[0] === "Il Ragazzo" || d[0] === "Il Mercenario") {
        tris.style.display = "flex"; controllo.style.display = "none";
        var fotoSrc = (d[0] === "Il Ragazzo") ? "https://res.cloudinary.com/dqoncufhc/image/upload/v1768446403/orfano_yd4ved.png" : "https://res.cloudinary.com/dqoncufhc/image/upload/v1768535131/soldato_ctg4bz.png";
        tris.innerHTML = `<img src="${fotoSrc}" class="foto-tris" style="width:200px; height:auto; margin-top: 10px;">`;
    } else {
        tris.style.display = "none"; controllo.style.display = "none";
    }
    document.getElementById('miuModal').style.display = "block";
}

function regolaTris(valore) { 
    var foto = document.querySelectorAll('.foto-tris'); 
    foto.forEach(img => { img.style.width = valore + "px"; }); 
}
